...
import { getNouvellesInscriptionsData } from "@/app/(app)/dashboard/widget-actions";
...
export function WidgetNouvellesInscriptions({ scope }: { scope: DashboardScope }) {
  const [data, setData] = useState<{ last7: number; last30: number; prev7: number; prev30: number } | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    setLoading(true);
    getNouvellesInscriptionsData(scope).then(setData).finally(() => setLoading(false));
  }, [scope]);
